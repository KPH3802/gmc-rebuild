...
**Last updated:** 2026-05-12 UTC (P2-05 risk-control interfaces authorization slice; governance-only PR opened as draft, no implementation included)
...
**Phase 2 — Infrastructure foundation, narrowly opened at P2-01, P2-02, P2-03 (governance-only authorization slice), P2-04 (governance-only authorization slice), and P2-05 (governance-only authorization slice).**

Phase 1 (governance cleanup) was accepted per `1f101fc` (see §3). Phase 2 is **partially open**: Kevin has authorized PR P2-01 (package skeleton and test harness), PR P2-02 (minimal safe config schema with fake/example values only), PR P2-03 (UTC time utility — governance-only authorization slice; no implementation lands in the authorizing PR), PR P2-04 (structured logging and audit event conventions — governance-only authorization slice; no implementation lands in the authorizing PR), and PR P2-05 (risk-control interfaces — governance-only authorization slice; no implementation lands in the authorizing PR). P2-01 created the importable `src/gmc_rebuild/` layout with no runtime behavior; P2-02 adds the `src/gmc_rebuild/config/` submodule that exposes an immutable, safe-by-default project metadata object with no runtime behavior; P2-03 authorizes the future `src/gmc_rebuild/time/` UTC time utility submodule on the §8 step 4a allowlist, with the implementation deferred to a separate, narrowly scoped P2-03 implementation PR per the exit criteria in `governance/authorizations/2026-05-12_p2-03.md`; P2-04 authorizes the future `src/gmc_rebuild/logging/` structured-logging and audit-event submodule on the §8 step 4a allowlist, with the implementation deferred to a separate, narrowly scoped P2-04 implementation PR per the exit criteria in `governance/authorizations/2026-05-12_p2-04.md`; P2-05 authorizes the future `src/gmc_rebuild/risk/` risk-control interfaces submodule (ADR-002 / ADR-003 / ADR-005-aligned) on the §8 step 4a allowlist, with the implementation deferred to a separate, narrowly scoped P2-05 implementation PR per the exit criteria in `governance/authorizations/2026-05-12_p2-05.md`. See `plan/phase2_entry_plan.md` §4 for the full P2-01..P2-05 sequence; all of P2-01, P2-02, the P2-03 authorization slice, the P2-04 authorization slice, and the P2-05 authorization slice are open. P2-05 is the final task in the P2-01..P2-05 sequence; no Phase 3+ work is authorized. The durable in-tree records of these authorizations are at `governance/authorizations/2026-05-11_p2-01.md`, `governance/authorizations/2026-05-11_p2-02.md`, `governance/authorizations/2026-05-12_p2-03.md`, `governance/authorizations/2026-05-12_p2-04.md`, and `governance/authorizations/2026-05-12_p2-05.md`.
...
## 5. What Phase 1 (and the P2-01 / P2-02 Implementation, plus the P2-03, P2-04, and P2-05 Authorization Slices) Contains
...
- The P2-05 authorization slice: a governance-only authorization to open the future `src/gmc_rebuild/risk/` risk-control interfaces submodule (abstract Protocols / interfaces and supporting frozen dataclasses for the kill-switch, reconciliation, and heartbeat boundaries aligned with ADR-002, ADR-003, and ADR-005; types and abstract boundaries only; no broker integration, no concrete runtime behavior, no external sinks, no `__main__` entry point). The §8 step 4a allowlist is extended in the authorizing PR to include `src/gmc_rebuild/risk/`; the directory is **not** created and no implementation code is landed by the authorizing PR. Authorized by `governance/authorizations/2026-05-12_p2-05.md`.
...
- **PR P2-05 — Risk-control interfaces (governance-only authorization slice).** Authorizes adding `src/gmc_rebuild/risk/` to the §8 step 4a allowlist and lands the durable authorization artifact at `governance/authorizations/2026-05-12_p2-05.md`. No implementation, no module files, no tests, and no directory creation are included; the implementation must arrive in a separate, narrowly scoped P2-05 implementation PR governed by the exit criteria in that authorization. The implementation, when later authorized, is restricted to abstract Protocols / interfaces and supporting frozen dataclasses for the kill-switch (ADR-002), reconciliation (ADR-003), and heartbeat (ADR-005) boundaries, plus test-only fakes under `tests/`. Does not authorize trading signals, broker / account / order-management logic (no broker SDK, no order objects, no order placement), market-data ingestion, persistent storage, schedulers / background jobs / runtime daemons, external sinks, env-var loading inside the risk submodule, any concrete runtime implementation of the protocols inside the runtime package, any Phase 3+ behavior, or any of the always-forbidden categories above.
...
- The §8 step 4a allowlist currently contains exactly `src/` (authorizing P2-01's importable `src/gmc_rebuild/` skeleton), `src/gmc_rebuild/config/` (authorizing the P2-02 config schema submodule), `src/gmc_rebuild/time/` (authorizing the P2-03 UTC time utility submodule as a governance-only slice; the directory is not materialised by the authorizing PR and lands only with a separate P2-03 implementation PR), `src/gmc_rebuild/logging/` (authorizing the P2-04 structured-logging and audit-event submodule as a governance-only slice; the directory is not materialised by the authorizing PR and lands only with a separate P2-04 implementation PR), and `src/gmc_rebuild/risk/` (authorizing the P2-05 risk-control interfaces submodule as a governance-only slice; the directory is not materialised by the authorizing PR and lands only with a separate P2-05 implementation PR). No other Phase 2 infrastructure paths are authorized.
- P2-05 is the final task in `plan/phase2_entry_plan.md` §4 P2-01..P2-05 sequence. Any work beyond P2-05 — Phase 2 expansion to new tasks not in the sequence, or any Phase 3+ item — is **not** authorized. It requires its own written authorization from Kevin per `AI_WORKFLOW.md` §6 rule 3 ("One approver") and rule 7 ("No phase drift"), a sibling authorization artifact under `governance/authorizations/` per `AI_WORKFLOW.md` §7, and a corresponding update to the §8 step 4a allowlist in the PR that introduces the directory.
- The always-forbidden categories in §6 and `plan/phase2_entry_plan.md` §2 remain forbidden regardless of mode. Authorizing P2-01, P2-02, the P2-03 authorization slice, the P2-04 authorization slice, and the P2-05 authorization slice does not authorize any Phase 3+ behavior, does not authorize env-var loading of secrets, and does not authorize any runtime entry point. The P2-03, P2-04, and P2-05 authorization slices in particular authorize paths on the allowlist but do not authorize any code that would live under them; those implementations require separate P2-03, P2-04, and P2-05 implementation PRs respectively. The P2-05 implementation, when later authorized, is restricted to abstract Protocols / interfaces and supporting frozen dataclasses; no concrete runtime implementation that talks to a broker, network, filesystem, or scheduler lands under `src/gmc_rebuild/risk/`.
...
#     `plan/phase2_entry_plan.md` §4 P2-01..P2-05 sequence; extending the
#     allowlist for any task beyond P2-05 requires a separate written
#     authorization from Kevin, a sibling authorization artifact under
...
#     mode; switching modes never relaxes those categories. PR P2-03
#     authorizes the `src/gmc_rebuild/time/` infrastructure path on the
#     allowlist as a governance-only slice, PR P2-04 authorizes the
#     `src/gmc_rebuild/logging/` infrastructure path on the allowlist as a
#     governance-only slice, and PR P2-05 authorizes the
#     `src/gmc_rebuild/risk/` infrastructure path on the allowlist as a
#     governance-only slice; no implementation is landed by any of those
#     authorizing PRs, so the directories may not yet exist on disk at the
#     time §8 step 4a runs.
