...
P3-03 and P3-04 are the only two authorized concrete Phase 3 implementation slices; P3-03 is the HeartbeatProtocol in-memory test fixture (ADR-005) and P3-04 is the KillSwitchProtocol in-memory test fixture (ADR-002). No further Phase 3 task is authorized; any such work requires its own separate written authorization per `MASTER_STATUS.md` §7 and `AI_WORKFLOW.md` §6 rule 3 ("One approver") / rule 7 ("No phase drift"). A PR that introduces work outside the §8 step 4a allowlist must be rejected at review.
...
- The §8 step 4a allowlist currently contains exactly `src/` (authorizing P2-01's importable `src/gmc_rebuild/` skeleton), `src/gmc_rebuild/config/` (authorizing the P2-02 config schema submodule), `src/gmc_rebuild/time/` (authorizing the P2-03 UTC time utility submodule), `src/gmc_rebuild/logging/` (authorizing the P2-04 structured-logging and audit-event submodule), `src/gmc_rebuild/risk/` (authorizing the P2-05 risk-control interfaces submodule — types and abstract boundaries only), `src/gmc_rebuild/heartbeat/` (authorizing the P3-03 HeartbeatProtocol in-memory test fixture — pure-Python in-memory fake only, no runtime activation), and `src/gmc_rebuild/kill_switch/` (authorizing the P3-04 KillSwitchProtocol in-memory test fixture — pure-Python in-memory fake only, no runtime activation). All five P2-01..P2-05 tasks are merged on `main`. **Phase 2 is formally closed (governance-only) at the current `main` checkpoint after PR #23 per `governance/authorizations/2026-05-12_phase-2-closure.md`; the closure does not extend this allowlist.** The P3-03 implementation entry was added by the P3-03 implementation PR under `governance/authorizations/2026-05-13_p3-03.md` and the P3-04 implementation entry is added by the P3-04 implementation PR under `governance/authorizations/2026-05-14_p3-04.md`; together they are the only two authorized concrete Phase 3 slices, and no other Phase 2 or Phase 3 infrastructure paths are authorized.
...
for path in src src/gmc_rebuild/config src/gmc_rebuild/time src/gmc_rebuild/logging src/gmc_rebuild/risk src/gmc_rebuild/heartbeat src/gmc_rebuild/kill_switch; do
  if [ -e "$path" ]; then
...
[ "${p2_infra_found:-0}" -eq 0 ] && echo "OK: Phase 2/3 infrastructure paths conform to P2-01/P2-02/P2-03/P2-04/P2-05/P3-03/P3-04 allowlist"

# 4b. The Phase 2 / Phase 3 implementation-mode allowlist in step 4a now
#     contains the P2-01, P2-02, P2-03, P2-04, P2-05, P3-03, and P3-04
#     entries. P2-05 is the final task in the `plan/phase2_entry_plan.md`
#     §4 P2-01..P2-05 sequence and is merged; Phase 2 is formally closed.
#     P3-03 (HeartbeatProtocol in-memory test fixture) is the first
#     concrete Phase 3 implementation slice and is authorized only for
#     the `src/gmc_rebuild/heartbeat/` directory under
#     `governance/authorizations/2026-05-13_p3-03.md`. P3-04
#     (KillSwitchProtocol in-memory test fixture) is the second concrete
#     Phase 3 implementation slice and is authorized only for the
#     `src/gmc_rebuild/kill_switch/` directory under
#     `governance/authorizations/2026-05-14_p3-04.md`. Extending the
#     allowlist for any further task (any third protocol fixture, any
#     runtime activation, any task beyond P3-04) requires a separate
...
#     P3-03 in-memory fake lives under `src/gmc_rebuild/heartbeat/` and the
#     P3-04 in-memory fake lives under `src/gmc_rebuild/kill_switch/`, both
#     test-fixture support only (no runtime consumer, no `__main__`, no
#     daemon, no scheduler, no network, no broker, no persistence).
