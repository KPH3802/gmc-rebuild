...
**Last updated:** 2026-05-13 UTC (status reconciliation — records that the P3-03 HeartbeatProtocol in-memory fixture has merged on `main`: PR #43 merged first at `07aee55` as the Mode B monitoring packet for PR #42 per ADR-008 §D5, then PR #42 merged at `c5e868c` adding `src/gmc_rebuild/heartbeat/InMemoryHeartbeat` and `tests/heartbeat/test_heartbeat_fixture.py` and extending the §8 step 4a allowlist by exactly one entry — `src/gmc_rebuild/heartbeat`; PR #45 then merged at `70ebc8f` as the Mode B monitoring packet for PR #44, and PR #44 merged at `db6d6fb` (current `main` head) as lint remediation for the Heartbeat fixture tests (no `src/**` change, no allowlist change, no quality-gate change); Phase 3 remains open as a governance state per `governance/authorizations/2026-05-13_p3-02.md`; Phase 2 remains formally closed; the §8 step 4a allowlist has exactly six entries — the five P2-01..P2-05 entries plus the single P3-03 `src/gmc_rebuild/heartbeat` entry — and is not extended further by this reconciliation; no second protocol fixture is authorized, no runtime activation is authorized, no quality gate is relaxed, no broker / market-data / order / strategy / scheduler / persistence / deployment / env-var / secrets change is introduced, and no tag or release is created)
...
- The §8 step 4a allowlist currently contains exactly `src/` (authorizing P2-01's importable `src/gmc_rebuild/` skeleton), `src/gmc_rebuild/config/` (authorizing the P2-02 config schema submodule), `src/gmc_rebuild/time/` (authorizing the P2-03 UTC time utility submodule), `src/gmc_rebuild/logging/` (authorizing the P2-04 structured-logging and audit-event submodule), `src/gmc_rebuild/risk/` (authorizing the P2-05 risk-control interfaces submodule — types and abstract boundaries only), and `src/gmc_rebuild/heartbeat/` (authorizing the P3-03 HeartbeatProtocol in-memory test fixture — pure-Python in-memory fake only, no runtime activation). All five P2-01..P2-05 tasks are merged on `main`. **Phase 2 is formally closed (governance-only) at the current `main` checkpoint after PR #23 per `governance/authorizations/2026-05-12_phase-2-closure.md`; the closure does not extend this allowlist.** The P3-03 implementation entry was added by the P3-03 implementation PR under `governance/authorizations/2026-05-13_p3-03.md` and is the first concrete Phase 3 slice; no other Phase 2 or Phase 3 infrastructure paths are authorized.
...
for path in src src/gmc_rebuild/config src/gmc_rebuild/time src/gmc_rebuild/logging src/gmc_rebuild/risk src/gmc_rebuild/heartbeat; do
  if [ -e "$path" ]; then
...
[ "${p2_infra_found:-0}" -eq 0 ] && echo "OK: Phase 2/3 infrastructure paths conform to P2-01/P2-02/P2-03/P2-04/P2-05/P3-03 allowlist"

# 4b. The Phase 2 / Phase 3 implementation-mode allowlist in step 4a now
#     contains the P2-01, P2-02, P2-03, P2-04, P2-05, and P3-03 entries.
#     P2-05 is the final task in the `plan/phase2_entry_plan.md` §4
#     P2-01..P2-05 sequence and is merged; Phase 2 is formally closed. P3-03
#     (HeartbeatProtocol in-memory test fixture) is the first concrete
#     Phase 3 implementation slice and is authorized only for the
#     `src/gmc_rebuild/heartbeat/` directory under
#     `governance/authorizations/2026-05-13_p3-03.md`. Extending the
#     allowlist for any further task (any second protocol fixture, any
#     runtime activation, any task beyond P3-03) requires a separate
...
#     P3-03 in-memory fake lives under `src/gmc_rebuild/heartbeat/` and is
