...
- **OPS-06 — Recurring backup verification and monitoring cadence.** Status: **plan documented at §17 by GOV-01; recurring cadence and escalation policy authorized as an operator commitment by OPS-06 (pending merge as of this packet)**. Authorization of record for the plan: `governance/authorizations/2026-05-17_gov-01.md`. Authorization of record for the cadence and escalation policy: `governance/authorizations/2026-05-17_ops-06.md`. Scope (per §17 plan and §17.7 status): the §17.2 nine-check read-only inspection set, the §17.3 escalation thresholds, the §17.4 escalation responses (operator-side recording for warnings; OPS-03-shape investigation packet for investigate / red signals; pause-coding for GitHub unreachable; demote for X10 Pro I/O errors), and the §17.5 cadence (daily / weekly / monthly / phase-boundary / immediate-after-incident) are now the operator's standing recurring procedure. **Each individual check execution and each individual restore drill remains an operator-side action.** OPS-06 does **not** authorize any monitoring automation, alerting system, notification mechanism, cron job, launchd plist, webhook, or chat integration (all explicitly deferred per §17.4 / §17.6 / §17.7); does **not** authorize any backup execution, restore execution, Backblaze setting change, Time Machine setting change, drive action, system setting change, or hardware change; does **not** promote the X10 Pro to Layer 5 (OPS-05 remains future / not authorized); and does **not** authorize any P5 execution-adjacent work. Individual restore drills and incident-triggered investigations continue to be recorded as dated §5.5 / §5.6 entries under their own separate written authorizations.
