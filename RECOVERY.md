...
- `governance/authorizations/2026-05-17_ops-06.md` — authorization of record for the OPS-06 recurring backup-verification and monitoring cadence (the GOV-01 §17 plan converted to a standing operator commitment), reflected at §13, §16.1, and §17.7.
...
- **OPS-06 — Recurring backup verification and monitoring cadence.** Status: **plan documented at §17 by GOV-01 (merged in PR #106); recurring cadence and escalation policy authorized as an operator commitment by OPS-06 (merged on `main` at `bc7aa65` on 2026-05-17)**. Authorization of record for the plan: `governance/authorizations/2026-05-17_gov-01.md`. Authorization of record for the cadence and escalation policy: `governance/authorizations/2026-05-17_ops-06.md`. Scope (per §17 plan and §17.7 status): the §17.2 nine-check read-only inspection set, the §17.3 escalation thresholds, the §17.4 escalation responses (operator-side recording for warnings; OPS-03-shape investigation packet for investigate / red signals; pause-coding for GitHub unreachable; demote for X10 Pro I/O errors), and the §17.5 cadence (daily / weekly / monthly / phase-boundary / immediate-after-incident) are now the operator's standing recurring procedure. **Each individual check execution and each individual restore drill remains an operator-side action.** OPS-06 does **not** authorize any monitoring automation, alerting system, notification mechanism, cron job, launchd plist, webhook, or chat integration (all explicitly deferred per §17.4 / §17.6 / §17.7); does **not** authorize any backup execution, restore execution, Backblaze setting change, Time Machine setting change, drive action, system setting change, or hardware change; does **not** promote the X10 Pro to Layer 5 (OPS-05 remains future / not authorized); and does **not** authorize any P5 execution-adjacent work. Individual restore drills and incident-triggered investigations continue to be recorded as dated §5.5 / §5.6 entries under their own separate written authorizations.
...
- **OPS-06 — Recurring backup-verification and monitoring cadence.** Status: **plan documented as §17 below (merged in PR #106); recurring cadence and escalation policy authorized as an operator commitment by OPS-06 (merged on `main` at `bc7aa65` on 2026-05-17) per `governance/authorizations/2026-05-17_ops-06.md` and recorded at §17.7**. Trigger: any operator-side execution of the §17 checks; any material change to the §17 plan; any new automation, alerting, or notification proposal; any new restore drill; any new incident-triggered investigation. Rule: §17 defines the per-check thresholds and the escalation matrix; OPS-06 commits the operator to the §17.5 cadence and to the §17.4 response policy. Individual check runs and individual restore drills remain operator-side actions; alerting, notification, automation, cron jobs, launchd plists, webhooks, chat integrations, Backblaze / Time Machine setting changes, drive actions, and X10 Pro Layer 5 promotion are explicitly **not authorized** by OPS-06 — each requires its own separate written authorization. Owner: Kevin.
...
### 17.7 Status as of OPS-06

**OPS-06 authorization status (2026-05-17 — merged on `main` at `bc7aa65`):** the §17 plan defined by GOV-01 (`governance/authorizations/2026-05-17_gov-01.md`) is **authorized as a recurring operator commitment** by `governance/authorizations/2026-05-17_ops-06.md`. The commitment converts §17 from "documented plan with recommended cadence" into the operator's standing recurring procedure: the §17.2 nine-check read-only inspection set, the §17.3 escalation thresholds, the §17.4 escalation responses, and the §17.5 cadence (daily / weekly / monthly / phase-boundary / immediate-after-incident) are now the agreed manual procedure. Each individual check run and each individual restore drill remains an operator-side action; OPS-06 commits to the *cadence and response policy*, not to any specific execution.

**What OPS-06 explicitly does not change from §17.6.** Every non-authorization in §17.6 above continues to apply unchanged. OPS-06 does **not** authorize:

- Any monitoring automation, cron job, launchd plist, desktop notification, email / SMS / push / webhook / chat integration, or any other alerting mechanism. Alerting / notification remains a future / not-authorized concern; if and when the operator wants any notification mechanism, it is its own separate written authorization packet that may extend §17.4 with the specific tool, scope, and audit trail.
- Any backup execution, restore execution, drive action, Backblaze setting change, Time Machine setting change, network configuration change, system setting change, app installation, or hardware change. The recurring checks are read-only by §17.2; OPS-06 does not relax that.
- Any X10 Pro Layer 5 promotion. The OPS-05 entry in §13 and the §16.1 OPS-05 Risk Register entry continue to govern Layer 5 promotion, which remains future / not authorized and continues to require resolution of the four remaining §5.6 gaps and Kevin's separate written authorization.
- Any clean-directory or clean-machine disaster-recovery drill execution. OPS-07 remains future / not authorized; the §17.5 "at every phase boundary" cadence references OPS-07 as the named follow-up, but does not pre-authorize the drill itself.
- Any Phase 5 execution-adjacent work, paper- or live-broker integration, market-data ingestion, order placement, persistence, scheduler / daemon, or any other forbidden surface from `RECOVERY.md` §7 / §16. The §17 checks are an observability layer; they do not unlock any P5-03+ implementation work, do not bypass the P4 safety gates, and do not change the inert character of the merged `src/gmc_rebuild/simulation/` subpackage.

**Re-baseline / re-authorization trigger.** OPS-06 authorizes the §17 plan **as it exists at the time of this packet**. If §17 is materially changed by a future docs/governance-only packet (a new check, a new threshold, a new response category, a new cadence row, a new "what does not do" non-authorization, or any other substantive change), the OPS-06 cadence commitment must be re-authorized by a successor packet that names the §17 change and accepts the resulting commitment. Trivial typo fixes, cross-reference updates, or formatting fixes do not require re-authorization; substantive changes do. The judgment call is the operator's at the time of the §17 change.

**Relationship to OPS-05 / OPS-07 / restore drills.** OPS-06 is the *observability* commitment; OPS-05 and OPS-07 remain *change* and *drill* commitments respectively and are unchanged in scope by this packet:

- OPS-05 (X10 Pro Layer 5 promotion) is gated by the four §5.6 / §10 gaps and continues to require its own separate written authorization. OPS-06's §17.2 check 7 includes the X10 Pro mount / offline-intent status as part of the recurring check set; observing it does not promote it. OPS-06's "demote" escalation (§17.3 "X10 Pro storage / I/O errors") strengthens OPS-05's gating by adding an explicit observability signal that would block promotion until reviewed.
- OPS-07 (clean-directory or clean-machine disaster-recovery drill) remains future / not authorized. OPS-06's §17.5 cadence references OPS-07 ("at every phase boundary") as the named follow-up but does not pre-authorize the drill execution. Each OPS-07 execution would be its own separate written authorization recorded as a dated §5.6 entry.
- Recurring restore drills (§17.2 check 9, recommended monthly per §17.5) follow the OPS-02 pattern: each drill is a separate operator-side action that picks one small non-sensitive file, restores it byte-for-byte from the relevant layer to an off-repo location, verifies via `shasum -a 256`, and records the result as a new dated `RECOVERY.md` §5.5 entry under its own future written authorization. OPS-06 commits to the *cadence* of recurring drills but does not pre-authorize any specific drill.

**Future automation / notification additions.** Adding any future automation (cron job, launchd plist, small operator-side script outside this repository) or any future notification mechanism (desktop notification, email, SMS, push, webhook, chat integration) is **not authorized** by OPS-06 and is **not implicitly enabled** by the cadence commitment. Each such addition is its own future operator-side action that requires its own separate written authorization, its own scope statement, its own audit-trail expectation (especially if any notification mechanism crosses any external service), and — if it adds any new file under this repository — its own pre-commit / mypy / detect-secrets validation. The default posture remains: this repository performs no scheduling, no automation, no network call, no notification.

**Backup-failure definition.** For the purposes of OPS-06, a "backup failure" is any §17.3 *Investigate (red)* threshold breach: Time Machine latest backup older than 48 hours or `tmutil destinationinfo` reporting no destination; Backblaze no successful backup in the last 72 hours or `bzserv` not running; Backblaze remaining backlog increasing across three consecutive checks or absolute remaining bytes growing beyond 2× the prior check; any new `loginwindow` `previousStartupWasAPanicOrHardRestart` entry or new `*.panic` / `Kernel_*` artifact; two or more new X10 Pro storage / I/O kernel-log entries in a rolling 7-day window; or any GitHub reachability failure that persists past the next scheduled check. A *Warning (yellow)* breach is not a failure; it is a signal to record and re-check. A *Green* result is the normal-state condition. The operator's response to a failure is the §17.4 escalation: open an OPS-03-shape investigation packet under separate written authorization, record findings at §5.6, and do not bypass the §17.6 non-authorizations as part of the investigation.

**Status of the OPS-06 Risk Register entry (§16.1).** Concurrent with this packet, the §16.1 OPS-06 entry is updated to reflect the authorized cadence and to flag the carry-forward future controls (alerting / notification policy, automation policy, recurring restore-drill cadence formalization) that remain open. The entry stays open in the Risk Register because the carry-forward items are not closed by OPS-06; only the cadence commitment is.
