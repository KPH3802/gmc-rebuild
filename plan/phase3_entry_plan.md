...
**§8 step 4a allowlist note.** The `allowed_p2_infra` allowlist currently contains exactly six entries (`src`, `src/gmc_rebuild/config`, `src/gmc_rebuild/time`, `src/gmc_rebuild/logging`, `src/gmc_rebuild/risk`, `src/gmc_rebuild/heartbeat`); the sixth entry was added by PR #42 (P3-03 implementation) and is unchanged by PR #44 (lint remediation, `tests/heartbeat/` only). This plan does **not** extend that allowlist further. Any future Phase 3 PR that introduces a new directory must add it to the allowlist in the same PR that introduces the directory, per the standing rule in `MASTER_STATUS.md` §8 step 4b, and must be authorized in writing for the specific directory by Kevin.
