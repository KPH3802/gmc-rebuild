...
- **P4-02 implementation (open / pending merge):** Authorized by Kevin on 2026-05-14 in writing per `governance/authorizations/2026-05-14_p4-02.md` as the **first Phase 4 implementation task**, narrowly scoped to a pure-Python, in-memory, pytest-only **composed-fixture test** exercising `InMemoryHeartbeat`, `InMemoryKillSwitch`, and `InMemoryReconciliation` together against their abstract `typing.Protocol` boundaries declared in `src/gmc_rebuild/risk/`. The verbatim written authorization is reproduced in full in that artifact and is the authorization of record per `AI_WORKFLOW.md` §7. The implementation PR is **open and pending merge**; per ADR-008 §D3 / §D5 (in force per ADR-009 D7's bootstrap-avoidance clause until runtime exists on `main`) and the verbatim authorization, the Mode B monitoring packet for the active workday is opened as a separate sibling monitoring PR and **must merge first** before the implementation PR merges. Mode A adversarial review per `AI_WORKFLOW.md` §4(2) (high-risk architecture decision — first concrete Phase 4 behaviour behind a P2-05 control surface, composed) is required and delivered as PR-review text on the implementation PR (not committed). The implementation adds a new `tests/p4_02_composed/` test-only directory (composed-fixture test only) and the durable authorization artifact at `governance/authorizations/2026-05-14_p4-02.md`, plus conservative governance/status reflections in `MASTER_STATUS.md`, `README.md`, and this plan. It does **not** add any new `src/**` directory or file, **does not** modify any file under `src/gmc_rebuild/risk/`, **does not** re-export any merged Phase 3 fixture from any runtime path, **does not** extend the `MASTER_STATUS.md` §8 step 4a allowlist (the eight P2-01..P2-05 + P3-03 + P3-04 + P3-05 entries are preserved exactly), **does not** relax any quality gate, and **does not** authorize any runtime activation, `__main__`, daemon, scheduler, broker, market-data, order, strategy, persistence, deployment, env-var, secrets, network, `time.sleep`, concrete protocol implementation, allowlist expansion, tag, or release. P4-03 / P4-04 / … remain future / not authorized.
...
2. **PR P4-02 — first Phase 4 implementation task (authorized 2026-05-14; open / pending merge).** Authorized by Kevin on 2026-05-14 in writing per `governance/authorizations/2026-05-14_p4-02.md` in a **narrow shape**: a pure-Python, in-memory, pytest-only composed-fixture test exercising `InMemoryHeartbeat`, `InMemoryKillSwitch`, and `InMemoryReconciliation` together against their abstract `typing.Protocol` boundaries declared in `src/gmc_rebuild/risk/`. The implementation lives only under a new `tests/p4_02_composed/` directory; it does **not** add any new `src/**` directory, does **not** modify `src/gmc_rebuild/risk/`, does **not** re-export any merged Phase 3 fixture from any runtime path, and does **not** extend the §8 step 4a allowlist (preserved exactly at eight entries). All other categories named in this item below (runtime activation, broker, market-data, order, strategy, scheduler, persistence, deployment, env-var, secrets, network, `time.sleep`, concrete protocol implementation, allowlist expansion, tag, release) remain explicitly **not authorized** by the verbatim P4-02 written authorization; each remains future / not authorized and would require its own separate written authorization from Kevin. Historically, this entry recorded P4-02 only as a candidate future task:
