...
2. **PR P2-02 — Config schema with fake examples only.** Add a config schema module (dataclasses-based) with safe local-only defaults. Frozen `ProjectConfig` with metadata fields only (`project_name`, `package_name`, `phase`, `environment`, `local_data_dir`, `local_logs_dir`) plus a `default_config()` helper. No real credentials, no real endpoints, no real account identifiers, no real broker / venue names, no live or paper toggles, no runtime-behavior boolean flags (even disabled), no env-var loading, no filesystem materialisation. The `local_data_dir` default is `./gmc_data` so it cannot collide with the §8 step 4 always-forbidden top-level `data` path. Tests cover importability, frozen-ness, safe defaults, the absence of any runtime-behavior boolean toggle, the absence of forbidden runtime submodules, and the absence of forbidden field tokens. **Authorized and opened as a draft PR** (durable record at `governance/authorizations/2026-05-11_p2-02.md`). The §8 step 4a allowlist is extended in the same PR to include `src/gmc_rebuild/config/` alongside the existing `src/`. Schema-validation tests against malformed inputs and additional field types are deferred to a follow-up PR if they are needed; P2-02 stays minimal.
3. **PR P2-03 — UTC and time utility module.** Add a small module that exposes the canonical "now UTC" function, parsers, and formatters, and forbids timezone-naive datetimes at the API boundary. Aligns with ADR-004. Tests cover round-trip and rejection of naive inputs. **Authorized as a governance-only slice** (durable record at `governance/authorizations/2026-05-12_p2-03.md`): the authorizing PR extends the §8 step 4a allowlist to include `src/gmc_rebuild/time/` alongside the existing `src/` and `src/gmc_rebuild/config/`, and lands the authorization artifact. **No implementation code, no module files, no tests, and no directory creation are included in the authorizing PR.** The implementation lands in a separate, narrowly scoped P2-03 implementation PR under that authorization's exit criteria.
4. **PR P2-04 — Structured logging and audit event conventions.** Add a logging configuration module and an `audit_event` helper that produces structured records with documented fields. No external sink; output is to the standard logger only. Tests cover record shape and redaction. **Authorized as a governance-only slice** (durable record at `governance/authorizations/2026-05-12_p2-04.md`): the authorizing PR extends the §8 step 4a allowlist to include `src/gmc_rebuild/logging/` alongside the existing `src/`, `src/gmc_rebuild/config/`, and `src/gmc_rebuild/time/`, and lands the authorization artifact. **No implementation code, no module files, no tests, and no directory creation are included in the authorizing PR.** The implementation lands in a separate, narrowly scoped P2-04 implementation PR under that authorization's exit criteria.
5. **PR P2-05 — Risk-control interfaces (no broker).** Add abstract interfaces / protocols for kill switch, reconciliation, and heartbeat aligned with ADR-002, ADR-003, and ADR-005. Provide test-only fake implementations. No broker SDK, no network calls, no order objects. **Authorized as a governance-only slice** (durable record at `governance/authorizations/2026-05-12_p2-05.md`): the authorizing PR extends the §8 step 4a allowlist to include `src/gmc_rebuild/risk/` alongside the existing `src/`, `src/gmc_rebuild/config/`, `src/gmc_rebuild/time/`, and `src/gmc_rebuild/logging/`, and lands the authorization artifact. **No implementation code, no module files, no tests, and no directory creation are included in the authorizing PR.** The implementation lands in a separate, narrowly scoped P2-05 implementation PR under that authorization's exit criteria; that implementation is restricted to abstract Protocols / interfaces and supporting frozen dataclasses (no broker SDK, no network, no concrete runtime behavior inside the runtime package), with test-only fakes confined to `tests/`. P2-05 is the final task in this P2-01..P2-05 sequence; any work beyond P2-05 requires its own separate written authorization.
