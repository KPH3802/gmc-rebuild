...
2. **PR P2-02 — Config schema with fake examples only.** Add a config schema module (dataclasses-based) with safe local-only defaults. No real credentials, no real endpoints, no real account identifiers, no real broker / venue names, no live or paper toggles, no env-var loading. Tests cover importability, defaults, immutability, absence of forbidden runtime submodules, and absence of forbidden field names. **Authorized and opened as a draft PR** (durable record at `governance/authorizations/2026-05-11_p2-02.md`). The §8 step 4a allowlist is extended in the same PR to include `src/gmc_rebuild/config/` alongside the existing `src/`. Schema-validation tests against malformed inputs and additional field types are deferred to a follow-up PR if they are needed; P2-02 stays minimal.
