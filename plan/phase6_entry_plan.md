...
1. **PR P6-01 — candidate first Phase 6 implementation task (future / not authorized).**
...
2. **PR P6-02 — candidate second Phase 6 implementation task (future / not authorized).**
...
3. **PR P6-03 — candidate third Phase 6 implementation task (future / not authorized; planning / enumeration packet `governance/authorizations/2026-05-19_p6-03-planning.md` opened on 2026-05-19, recording the candidate shape at the planning level only and **not** authorizing implementation).**
   ...
   - **Docs-only or implementation:** Implementation. The P6-03 planning / enumeration packet at `governance/authorizations/2026-05-19_p6-03-planning.md` is the planning-only precursor and **does not** authorize implementation.
   ...
   - **Planning packet cross-reference:** `governance/authorizations/2026-05-19_p6-03-planning.md` §3 records the planning-level candidate shape (intended module/package location, typed input boundary from P6-01 / P6-02, typed output boundary, reason taxonomy, deterministic pure-function contract, safety / non-goals, proposed test matrix and invariants, integration expectations, acceptance criteria, and validation commands). The planning packet is **planning only** and does not pre-commit any element of the future P6-03 implementation authorization; Kevin's future P6-03 implementation authorization is free to replace any element of that candidate shape.

4. **PR P6-04 — candidate fourth Phase 6 implementation task (future / not authorized).**
   - **Objective (planning level):** Extend the existing `SimulatedOrderIntent` model into a richer dry-run order intent surface. Two planning-level candidate directions: (a) add deterministic intent-identifier generation and a time-in-force closed enum on the existing `SimulatedOrderIntent`; or (b) add a separately named record class that composes `SimulatedOrderIntent` with the P6-03 decision output to produce a "decided order intent." The chosen direction is itself a decision Kevin must make in writing at the time of authorization, and the authorization may replace both candidates with a third shape.
...
5. **PR P6-05 — candidate fifth Phase 6 implementation task (future / not authorized).**
   ...
   - **Touched directories (candidate):** New submodule under `src/gmc_rebuild/portfolio/` (or similarly named). New tests directory under `tests/portfolio/`.
   - **Docs-only or implementation:** Implementation.
   - **Expected tests (planning level):** Determinism of application, non-mutation of inputs, identity / equality semantics on the returned snapshot, idempotence under repeated application of the same accepted intent (or explicit non-idempotence if the chosen model is event-style; the authorization names the model), composed-pipeline integration with P6-01 / P6-02 / P6-03 / P6-04, inertness self-check.
   - **§8 step 4a allowlist implication:** Adds exactly one new entry `src/gmc_rebuild/portfolio` (or whatever the authorized name is) to the allowlist in the same PR.
   - **Explicit non-authorizations:** Does not authorize any real position book, any real account identifier, any broker reconciliation against a real broker, any persistence, any network call, any scheduler, any runtime activation. The portfolio remains purely in-memory and value-typed; no on-disk snapshot lands in this packet.
...
- **Any successor Phase 6 task.** P6-01 / P6-02 / … remain **future / not authorized**. Each requires its own separate written authorization and, if merely enumerated first, a separate docs-only enumeration update that does not authorize implementation.
...
## 11. Recommended First Dry-Run Implementation Packet (Not Opened)

The recommended first dry-run implementation packet is **PR P6-01 — the typed signal-intake boundary** (§4 above), corresponding to Roadmap §8 Packet 3. The reasoning recorded here at planning level only:
...
**This recommendation does not open P6-01.** Opening P6-01 requires Kevin's separate written authorization per `MASTER_STATUS.md` §7.3, recorded in a sibling artifact under `governance/authorizations/` per `AI_WORKFLOW.md` §7, with Mode A and Mode B as applicable per `AI_WORKFLOW.md` §4 and ADR-008 / ADR-009. Until that authorization is recorded, P6-01 remains **future / not authorized**.
