...
**P3-03 implementation has been authorized by Kevin in writing on 2026-05-13 per `governance/authorizations/2026-05-13_p3-03.md`** as the first concrete Phase 3 implementation slice, narrowly scoped to a pure-Python in-memory test fixture for `HeartbeatProtocol` (ADR-005) only. The implementation PR adds `src/gmc_rebuild/heartbeat/` (in-memory `InMemoryHeartbeat` fake — no runtime activation, no `__main__`, no daemon, no scheduler, no broker SDK, no network, no `time.sleep`, no `os.environ` / `os.getenv`, no filesystem, no real secrets / accounts / venues / endpoints) and `tests/heartbeat/test_heartbeat_fixture.py` (focused, deterministic test module exercising protocol-shape conformance and ADR-005 safe-default and staleness semantics against the abstract `HeartbeatProtocol` from `src/gmc_rebuild/risk/`). The implementation PR extends the `MASTER_STATUS.md` §8 step 4a allowlist by exactly one entry — `src/gmc_rebuild/heartbeat` — with no other allowlist change. The authorization does **not** authorize a second protocol fixture (for `KillSwitchProtocol` or `ReconciliationProtocol`), does **not** authorize any runtime activation, does **not** authorize any broker / market-data / order / strategy / scheduler / persistence / deployment / env-var / secrets change, and does **not** create any tag or release. The P3-03 implementation PR requires Mode A adversarial review per `AI_WORKFLOW.md` §4(2) (high-risk architecture decision — first concrete behaviour behind a P2-05 control surface) and a separate sibling Mode B monitoring packet merged to `main` before this implementation PR merges per ADR-008 §D3 / §D5.
