...
Phase 1 (governance cleanup) was accepted by Kevin in writing on PR #3 against commit `1f101fc`. Phase 2 implementation is **partially open**: Kevin has authorized PR P2-01 (package skeleton and test harness, see `plan/phase2_entry_plan.md` §4), PR P2-02 (minimal safe config schema), PR P2-03 (UTC time utility — governance-only authorization slice; no implementation lands in the authorizing PR), PR P2-04 (structured logging and audit event conventions — governance-only authorization slice; no implementation lands in the authorizing PR), and PR P2-05 (risk-control interfaces — governance-only authorization slice; no implementation lands in the authorizing PR). P2-01 created the importable `src/gmc_rebuild/` layout with no runtime behavior; P2-02 adds the `src/gmc_rebuild/config/` submodule that exposes a frozen `ProjectConfig` dataclass with safe local-only defaults and no runtime behavior; the P2-03 authorization slice extends the §8 step 4a allowlist to include the future `src/gmc_rebuild/time/` UTC time utility submodule (ADR-004-aligned) but lands no code, no directory, and no tests; the P2-04 authorization slice extends the §8 step 4a allowlist to include the future `src/gmc_rebuild/logging/` structured-logging and audit-event submodule but lands no code, no directory, and no tests; the P2-05 authorization slice extends the §8 step 4a allowlist to include the future `src/gmc_rebuild/risk/` risk-control interfaces submodule (ADR-002 / ADR-003 / ADR-005-aligned; abstract Protocols and supporting frozen dataclasses only, no broker integration, no concrete runtime behavior) but lands no code, no directory, and no tests. P2-05 is the final task in the `plan/phase2_entry_plan.md` §4 P2-01..P2-05 sequence; no other Phase 2 task and no Phase 3+ work is authorized. Strategy code, broker execution, live or paper trading wired to a real broker, runtime daemons affecting accounts, real market data ingestion, order placement, and real secrets remain forbidden — see `MASTER_STATUS.md` §6.

The P2-03 implementation PR (which lands the actual UTC time utility module under the path authorized for it) is gated by the exit criteria in `governance/authorizations/2026-05-12_p2-03.md`. The P2-04 implementation PR (which lands the actual structured-logging and audit-event module under the path authorized for it) is gated by the exit criteria in `governance/authorizations/2026-05-12_p2-04.md`. The P2-05 implementation PR (which would later land the abstract risk-control interfaces module under the path authorized here) is gated by the exit criteria in `governance/authorizations/2026-05-12_p2-05.md`.
...
| `src/gmc_rebuild/risk/` | Risk-control interfaces submodule (authorization-only slice under PR P2-05 — path added to the §8 step 4a allowlist; not materialised yet; abstract Protocols and supporting frozen dataclasses for the kill-switch (ADR-002), reconciliation (ADR-003), and heartbeat (ADR-005) boundaries only, no broker integration; implementation deferred to a separate P2-05 implementation PR per `governance/authorizations/2026-05-12_p2-05.md`) |
...
Phase 2 entry criteria (status as of the P2-05 authorization slice):
...
- Kevin explicitly authorized Phase 2 implementation, narrowly scoped to PR P2-01, PR P2-02, the P2-03 authorization slice, the P2-04 authorization slice, and the P2-05 authorization slice. **Satisfied for P2-01, P2-02, the P2-03 authorization slice, the P2-04 authorization slice, and the P2-05 authorization slice only.**
- The first two Phase 2 tasks are infrastructure-only (package skeleton and test harness; minimal safe config schema; per `plan/phase2_entry_plan.md` §4). **Satisfied.** The P2-03 authorization slice adds `src/gmc_rebuild/time/` to the §8 step 4a allowlist with no implementation; the P2-03 implementation PR remains gated. The P2-04 authorization slice adds `src/gmc_rebuild/logging/` to the §8 step 4a allowlist with no implementation; the P2-04 implementation PR remains gated. The P2-05 authorization slice adds `src/gmc_rebuild/risk/` to the §8 step 4a allowlist with no implementation; the P2-05 implementation PR remains gated.

P2-05 is the final task in the `plan/phase2_entry_plan.md` §4 P2-01..P2-05 sequence. Any task beyond P2-05 — Phase 2 expansion or any Phase 3+ item — requires its own written authorization; the above criteria do not auto-extend to it. The P2-03 implementation PR is governed by the exit criteria in `governance/authorizations/2026-05-12_p2-03.md`, the P2-04 implementation PR by `governance/authorizations/2026-05-12_p2-04.md`, and the P2-05 implementation PR by `governance/authorizations/2026-05-12_p2-05.md`.
