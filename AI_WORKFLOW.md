...
4. **One branch per task.** Codex works on a feature branch named for the task. No direct commits to the default branch. This rule applies prospectively from the adoption of `AI_WORKFLOW.md` (the merge commit that introduced this file on `main`); pre-adoption history on the default branch is not retroactively invalidated by this rule, but every change after adoption goes through a feature branch and PR.
