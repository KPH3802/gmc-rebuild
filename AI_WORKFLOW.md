...
The Backup AI operates in two distinct modes with non-overlapping triggers. Both modes share the same adversarial stance and the same safety boundary on what the Backup AI may produce; the trigger and the artifact differ. See `docs/decisions/ADR-008_monitoring_cadence_and_ai_monitor_role.md` for the full decision.

- **Mode A — Gate Reviewer.** Used **only** in the three situations defined in Section 4: phase gates, high-risk architecture decisions, and safety-critical decisions. Reviews adversarially: looks for ways the proposed change can fail, violate an invariant, or expand scope. Output is a written critique. Kevin reads it. Codex addresses it. Perplexity Computer re-verifies. Mode A produces no file under version control.
- **Mode B — Continuous Governance Monitor.** Used when a monitoring packet is required under the cadence rule in ADR-008 §D3. The Backup AI reads the working tree and recent PR activity and **authors the text** of a monitoring packet for `monitoring/daily/YYYY-MM-DD.md` using the existing template. The packet is **committed to the repository by Codex** (or, in routine governance maintenance, by whoever is acting as Codex on the task) under their authorship. Mode B does **not** authorize the Backup AI to push, merge, or commit on its own. Producing a monitoring packet is **not** approval and does **not** substitute for Perplexity Computer's verification or Kevin's decision.

In both modes the Backup AI does **not** write code, source files, configuration, trading logic, or governance decisions; does **not** build; does **not** decide; does **not** merge; and does **not** edit files outside the monitoring packet text it authors in Mode B. Authoring a packet's text in Mode B is the **only** content the Backup AI may produce that lands in the working tree, and even then only via Codex's commit — never directly. This preserves §6 rule 1 ("One builder at a time"): Codex remains the only role that writes to the repository.
...
Routine governance documentation edits **do not** trigger Mode A (per the routine-exclusion sentence above), but they **do** trigger Mode B when the ADR-008 §D3 conditions are met. The Mode A and Mode B triggers are independent.

Mode B is governed entirely by ADR-008. The packet location, format, the externally observable definition of "active workday", and missed-packet handling are defined there. Mode B does not bypass §3 verification or §1.3 approval. The Backup AI authors the packet text; Codex commits it (see §1.4).

**Overlap — when both Mode A and Mode B fire on the same PR.** Phase-gate, high-risk-architecture, and safety-critical PRs trigger Mode A; if they also meet the ADR-008 §D3 conditions (and most will), Mode B fires too. In that case, **two distinct artifacts are produced**: (1) the Mode A written critique (not committed to the repo, delivered to Kevin and Codex as a review), and (2) the Mode B monitoring packet (committed by Codex under `monitoring/daily/`). The packet may **link to or quote** the Mode A critique for context, but does **not** replace it: the critique is the adversarial review of the decision, the packet is the dated governance artifact of the workday. Producing only one of the two when both are required is a §6 rule 9 ("Every claim has evidence") failure.
...
1. **One builder at a time.** Only Codex writes to the repository in a given task. The Backup AI never writes code, source files, configuration, trading logic, or governance decisions, and never commits or pushes to the repository on its own. The one narrow exception is Mode B (see §1.4 and ADR-008): the Backup AI may **author the text** of a monitoring packet under `monitoring/daily/`, but Codex is still the role that **commits** that text to the repository. This exception does not relax "One builder at a time" — Codex remains the only role that writes to the repo. Perplexity Computer never silently rewrites Codex's work — it returns findings.
