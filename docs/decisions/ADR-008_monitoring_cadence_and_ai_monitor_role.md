...
- [x] Future ADR: define runtime monitoring cadence and missed-packet severity (D6). Owner: whoever opens the first runtime PR. Target phase: before any Phase 2 PR that introduces runtime (daemon, broker integration, market data ingestion, paper or live trading). *(Accepted as `docs/decisions/ADR-009_runtime_monitoring_cadence.md` under `governance/authorizations/2026-05-13_p3-01-acceptance.md`; drafted in Proposed status on 2026-05-13 under `governance/authorizations/2026-05-13_p3-01.md` and revised in Proposed status by PR #32 to address Mode A blocking findings B1–B5. ADR-009 D3 and D5 replace this ADR's §D3 and §D5 runtime-phase guidance on a going-forward basis once runtime exists on `main`; ADR-008 itself is **not** amended by ADR-009.)*
